...
class perf(Thread):
    def __init__(self):
        Thread.__init__(self)
        print("Initialisation du Thread de perf ")
        self.cpt=0
        self.continu=0
...
def camstream():
    ...
    camera.start()
    camera.set_controls(hflip = True, vflip = True,brightness=50)
        ...
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN:
                if event.key == K_ESCAPE: 
                    ...
                    capture = False
        if  GPIO.event_detected(BUTTON_PIN) or MODE=="forced":
            if MODE=="" or MODE=="forced":
               MODE="TEMPO"
